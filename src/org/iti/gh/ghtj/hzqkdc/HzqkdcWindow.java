...
import org.iti.gh.ghtj.QkdcDetailWindow;
import org.iti.gh.ghtj.QkdcItem;
...
public class HzqkdcWindow extends BaseWindow {
	
	Listbox condition,xlist;
	Label xname;
	Long kdid;
...
//	WkTUser user;
	...
	public void initShow() {
		...
		condition.setItemRenderer(new ListitemRenderer(){

			public void render(Listitem item, Object data) throws Exception {
				String tjx=(String)data;
				item.setValue(tjx);
				item.setLabel(tjx);
			}
			
		});
		condition.addEventListener(Events.ON_SELECT, new EventListener(){
		...
		xlist.setItemRenderer(new ListitemRenderer(){
	...
	public void initWindow() {
	    ...
	    String yname=getXyUserRole().getDept().getGradeName(WkTDept.GRADE_YUAN);
	    String zyname=getXyUserRole().getDept().getGradeName(WkTDept.GRADE_XI);
	    List conlist=new ArrayList();List deplist=new ArrayList();
	    conlist.add("按"+yname.trim()+"统计");
	    conlist.add("按"+zyname.trim()+"进行统计");
	    if(getXyUserRole().getDept().getKdLevel().intValue()==WkTDept.GRADE_XI){
	    	kdid=getXyUserRole().getDept().getKdPid();
	    }
//	    xname.setValue("选择"+zyname.trim());
	    deplist=departmentService.getChildDepdw(kdid);
		condition.setModel(new ListModelList(conlist));
		condition.setSelectedIndex(0);
		xlist.setModel(new ListModelList(deplist));
		xlist.setSelectedIndex(0);
		initDcWindow(kdid);
	}
	
	public void initDcWindow(Long Kdid){
		//科研项目国家
		...
		List kuidlist=qkdcService.getKuidByKdid(Kdid); 
		for(int i=0;i<kuidlist.size();i++){
			Long kuid=(Long)kuidlist.get(i);
			System.out.println(kuid.intValue()+" 这个教师");
...
//				System.out.println(kuid+"=="+ qk1115.getJxcgSb());
				...
				System.out.println(getXyUserRole().getUser().getKuName()+"情况调查列不全");
			}
		}
	...
	public void onClick$tj(){
		Long id;
        if(xlist.isVisible()){
        	id=((WkTDept)xlist.getSelectedItem().getValue()).getKdId();
        }else{
        	id=kdid;
        }
//        System.out.println(id.intValue()+"ppp");
		initDcWindow(id);
	}
...
//			System.out.println(value+" 不合格");
	...
	private void addMap(Toolbarbutton tb,QkdcItem qitem){
		Map<Toolbarbutton,List> temMap=new HashMap();
