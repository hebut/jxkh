...
public class AddVideoWin extends Window implements AfterCompose {
	...
	private Textbox name, videoMember, videoDept, coCompany, longTime, media;
	...
	public void afterCompose() {
		...
		// uploadbox1.initWindow();// 把上传的功能加载进来
		// uploadbox2.initWindow();// 把上传的功能加载进来
		...
		scoreTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (video != null) {
					if (video.getState() == null || video.getState() == 0 || video.getState() == 3 || video.getState() == 5 || video.getState() == 8) {
	...
	public void initWindow() {
		...
		if (video == null) {
			...
			// 判断如果是未审核状态则暂存按钮不显示
			if (video.getState() == null || video.getState() == JXKH_MEETING.WRITING || video.getState() == Jxkh_Project.BUSINESS_NOT_PASS || video.getState() == Jxkh_Project.DEPT_NOT_PASS)
				tempSave.setVisible(true);
			else
				tempSave.setVisible(false);
			if (video.getState() == Jxkh_Video.NOT_AUDIT || video.getState() == Jxkh_Video.DEPT_NOT_PASS || video.getState() == Jxkh_Video.BUSINESS_NOT_PASS || video.getState().shortValue() == JXKH_MEETING.WRITING) {
				save.setVisible(true);
	...
	public void onClick$tempSave() {
		...
		if (video == null) {
			...
			video.setSubmitId(user.getKuLid());
			video.setSubmitName(user.getKuName());
			video.setSubmitTime(ConvertUtil.convertDateAndTimeString(new Date()));
			setEntity();
		} else {
			setEntity();
		}
		video.setState(JXKH_MEETING.WRITING);
		try {
			jxkhVideoService.saveOrUpdate(video);
			Messagebox.show("影视保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			videoMemberList = video.getVideoMember();
	...
	public void onClick$save() {
		...
		video.setState(Jxkh_Video.NOT_AUDIT);
		try {
			jxkhVideoService.saveOrUpdate(video);
			Messagebox.show("影视已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			videoMemberList = video.getVideoMember();
			List<Jxkh_VideoDept> dlist = video.getVideoDept();
	...
	public void setEntity() {
		...
		if ((Jxkh_BusinessIndicator) leader.getSelectedItem().getValue() != null && leader.getSelectedIndex() != 0) {
		...
		video.setTempState("0000000");// 为临时状态赋初值
		video.setDep1Reason("");
		video.setDep2Reason("");
		video.setDep3Reason("");
		video.setDep4Reason("");
