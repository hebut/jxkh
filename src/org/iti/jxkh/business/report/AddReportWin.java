...
public class AddReportWin extends Window implements AfterCompose {
	...
	public void afterCompose() {
		...
		scoreTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (report != null) {
					if (report.getState() == null || report.getState() == 0 || report.getState() == 3 || report.getState() == 5 || report.getState() == 8) {
	...
	public void initWindow() {
		...
		if (report == null) {
			...
			// 判断如果是未审核状态则暂存按钮不显示
			if (report.getState() == null || report.getState() == JXKH_MEETING.WRITING || report.getState() == Jxkh_Project.BUSINESS_NOT_PASS || report.getState() == Jxkh_Project.DEPT_NOT_PASS)
				tempSave.setVisible(true);
			else
				tempSave.setVisible(false);
			if (report.getState() == Jxkh_Report.NOT_AUDIT || report.getState() == Jxkh_Report.DEPT_NOT_PASS || report.getState() == Jxkh_Report.BUSINESS_NOT_PASS || report.getState() == JXKH_MEETING.WRITING) {
				save.setVisible(true);
	...
	private void initListbox() {
		...
		String[] report_Types = { "", "调研报告", "分析报告", "其他" };
	...
	public void onClick$save() throws InterruptedException {
		...
		report.setState(JXKH_MEETING.NOT_AUDIT);
		jxkhReportService.saveOrUpdate(report);
		Messagebox.show("报告已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		scoreTab.setDisabled(false);
		fileTab.setDisabled(false);
	...
	public void onClick$tempSave() throws InterruptedException {
		...
		jxkhReportService.saveOrUpdate(report);
		Messagebox.show("报告保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		scoreTab.setDisabled(false);
		fileTab.setDisabled(false);
		/*List<Jxkh_ReportMember> mlist = report.getReportMember();
		List<Jxkh_ReportDept> dlist = report.getReprotDept();*/
