...
public class AddAwardWin extends Window implements AfterCompose {
	...
	private Textbox name, awardMember, awardDept, coCompany, authorizeCompany, registerCode;
	...
	public void afterCompose() {
		...
		// uploadbox1.initWindow();// 把上传的那个窗口加载进来
		// upload1.appendChild(uploadbox1);// 把上传的那个窗口加载进来
		// uploadbox2.initWindow();// 把上传的那个窗口加载进来
		// upload2.appendChild(uploadbox2);// 把上传的那个窗口加载进来
		...
		scoreTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (award != null) {
					if (award.getState() == null || award.getState() == 0 || award.getState() == 3 || award.getState() == 5 || award.getState() == 8) {
		...
		baseTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (detail == "Detail") {
					...
					if (award != null) {
						if (award.getState() != null
								&& (award.getState().equals(JXKH_MEETING.DEPT_PASS) || award.getState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS) || award.getState().equals(JXKH_MEETING.BUSINESS_PASS) || award.getState().equals(JXKH_MEETING.SAVE_RECORD))) {
							tempSave.setVisible(false);
							save.setVisible(false);
						} else {
	...
	public void initWindow() {
		...
		if (award == null) {
			...
			// 判断如果是未审核状态则暂存按钮不显示
			if (award.getState() == null || award.getState() == JXKH_MEETING.WRITING || award.getState() == Jxkh_Project.BUSINESS_NOT_PASS || award.getState() == Jxkh_Project.DEPT_NOT_PASS)
				tempSave.setVisible(true);
			else
				tempSave.setVisible(false);
			if (detail == "Detail") {
				...
				if (award.getState() == Jxkh_Award.NOT_AUDIT || award.getState() == Jxkh_Award.DEPT_NOT_PASS || award.getState() == Jxkh_Award.BUSINESS_NOT_PASS || award.getState().equals(JXKH_MEETING.WRITING)) {
					save.setVisible(true);
			...
			// ListModelList fileModel1 = new ListModelList(
			// new ArrayList<Object>());
			// for (int i = 0; i < file.length; i++) {
			// Jxkh_AwardFile f = (Jxkh_AwardFile) file[i];
			// File fileout = new File(Executions.getCurrent().getDesktop()
			...
			// }
			// }
			// uploadbox1.setFileModel(fileModel1);
			// uploadbox1.initShow();
			// // 初始化申报材料附件列表
			// ListModelList fileModel2 = new ListModelList(
			// new ArrayList<Object>());
			// for (int i = 0; i < file.length; i++) {
			// Jxkh_AwardFile f = (Jxkh_AwardFile) file[i];
			// File fileout = new File(Executions.getCurrent().getDesktop()
			...
			// Media media = new AMedia(fileout, null, "UTF-8");
			// fileModel2.add(media);
			// }
			// } catch (FileNotFoundException e) {
			// e.printStackTrace();
			// }
			// }
			// uploadbox2.setFileModel(fileModel2);
			// uploadbox2.initShow();
	...
	public void onClick$save() throws InterruptedException {
		...
		if (awardMember.getValue() == null || awardMember.getValue().equals("")) {
			Messagebox.show("请选择奖励成员！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			return;
		}
		if (awardDept.getValue() == null || awardDept.getValue().equals("")) {
			Messagebox.show("请选择奖励部门！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			return;
		}
		if (rank.getSelectedItem().getIndex() == 0) {
			Messagebox.show("请选择奖励等级！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			return;
		}
		...
		for (WkTUser user : memberList) {
			member = user.getKuName();
			if (this.user.getKuName().equals(member)) {
				i = 1;
			}
		}
		if (i == 0) {
			try {
				Messagebox.show("您不是完成人员，不能进行录入！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		...
		if (award == null) {
			...
			award.setSubmitTime(ConvertUtil.convertDateAndTimeString(new Date()));
			setEntity();
		} else {
			setEntity();
		}
		...
		try {
			...
			Messagebox.show("奖励已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			/*List<Jxkh_AwardMember> mlist = award.getAwardMember();?
			List<Jxkh_AwardDept> dlist = award.getAwardDept();*/
			awardMemberList = award.getAwardMember();
			awardDeptList = award.getAwardDept();
			personScore.setModel(new ListModelList(awardMemberList));
			departmentScore.setModel(new ListModelList(awardDeptList));
		} catch (Exception e) {
			e.printStackTrace();
			try {
				Messagebox.show("奖励保存失败，请重试！", "提示", Messagebox.OK, Messagebox.INFORMATION);
	...
	public void onClick$tempSave() throws InterruptedException {
		...
		try {
			jxkhAwardService.saveOrUpdate(award);
			Messagebox.show("奖励保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			/*List<Jxkh_AwardMember> mlist = award.getAwardMember();
			List<Jxkh_AwardDept> dlist = award.getAwardDept();*/
	...
	public void setEntity() throws InterruptedException {
		...
		if (rank.getSelectedItem().getIndex() != 0) {
			award.setRank((Jxkh_BusinessIndicator) rank.getSelectedItem().getValue());
	...
	public void onClick$chooseAward() throws SuspendNotAllowedException, InterruptedException {
		final ChooseFruitWin win = (ChooseFruitWin) Executions.createComponents("/admin/personal/businessdata/award/choosefruit.zul", null, null);
					...
					try {
						...
						w.addEventListener(Events.ON_CHANGE, new EventListener() {
							public void onEvent(Event arg0) throws Exception {
								...
								List<Jxkh_AwardDept> tempDeptList = jxkhAwardService.findAwardDeptByAwardId(award);
