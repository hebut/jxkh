...
public class ChooserProjectWin extends Window implements AfterCompose {
	...
	public void initListbox() {
		...
		List<Jxkh_Project> projectList = jxkhAwardService.findAllPByConditon(
				"", null);
	...
	public void onClick$query() {
		...
		List<Jxkh_Project> projectList = jxkhAwardService.findAllPByConditon(
				nameSearch, auditStateSearch);
		projectListbox.setModel(new ListModelList(projectList));
	}

	public void onClick$choose() {
		if (projectListbox.getSelectedItems() == null
				|| projectListbox.getSelectedItems().size() <= 0) {
			try {
				Messagebox.show("请选择项目！", "提示", Messagebox.OK,
