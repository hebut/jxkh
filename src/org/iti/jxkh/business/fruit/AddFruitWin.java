...
public class AddFruitWin extends Window implements AfterCompose {
	...
	private Textbox name, fruitMember, fruitDept, coCompany, appraiseCode, acceptCode, appraiseType, organAppraiseCompany, holdAppraiseCompany, acceptDept;
	...
	private Jxkh_Project project = new Jxkh_Project();
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (fruit != null) {
					if (detail == "Detail")
						...
						if (fruit.getState() == null || fruit.getState() == 0 || fruit.getState() == 3 || fruit.getState() == 5 || fruit.getState() == 8) {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (fruit != null) {
					if (detail == "Detail") {
						...
						if (fruit.getState() != null
								&& (fruit.getState().equals(JXKH_MEETING.DEPT_PASS) || fruit.getState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS) || fruit.getState().equals(JXKH_MEETING.BUSINESS_PASS) || fruit.getState().equals(JXKH_MEETING.SAVE_RECORD))) {
							tempSave.setVisible(false);
							save.setVisible(false);
						} else {
	...
	public void initWindow() {
		...
		if (fruit == null) {
			...
			// 判断如果是未审核状态则暂存按钮不显示
			if (fruit.getState() == null || fruit.getState() == JXKH_MEETING.WRITING || fruit.getState() == Jxkh_Project.BUSINESS_NOT_PASS || fruit.getState() == Jxkh_Project.DEPT_NOT_PASS)
				tempSave.setVisible(true);
			else
				tempSave.setVisible(false);
			if (detail == "Detail") {
				...
				if (fruit.getState() == Jxkh_Fruit.NOT_AUDIT || fruit.getState() == Jxkh_Fruit.DEPT_NOT_PASS || fruit.getState() == Jxkh_Fruit.BUSINESS_NOT_PASS || fruit.getState().equals(JXKH_MEETING.WRITING)) {
					save.setVisible(true);
	...
	public void onClick$save() throws InterruptedException {
		try {
			...
			if ((fruitRank.getSelectedItem().getIndex() != 0 || rate.getSelectedItem().getIndex() != 0) && (appraiseDate.getValue() == null || appraiseDate.getValue().equals(""))) {
			...
			for (WkTUser user : memberList) {
				System.out.println("长度是" + memberList.size());
				memberStr = user.getKuName();
				if (this.user.getKuName().equals(memberStr)) {
					k = 1;
				}
			}
			if (k == 0) {
			...
			setEntity();
			fruit.setState(JXKH_MEETING.NOT_AUDIT);
			jxkhFruitService.saveOrUpdate(fruit);
			if (project.getId() != null && !project.getProgress().equals("结题")) {
				project.setProgress("结题");
				jxkhAwardService.saveOrUpdate(project);
			}
			Messagebox.show("成果已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
	...
	public void onClick$tempSave() {
		try {
			...
			setEntity();
			fruit.setState(JXKH_MEETING.WRITING);
			jxkhFruitService.saveOrUpdate(fruit);
			if (project.getId() != null && !project.getProgress().equals("结题")) {
				project.setProgress("结题");
				jxkhAwardService.saveOrUpdate(project);
			}
			Messagebox.show("成果保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			/*List<Jxkh_FruitMember> mlist = fruit.getFruitMember();
			List<Jxkh_FruitDept> dlist = fruit.getFruitDept();*/
	...
	private void setEntity() throws InterruptedException {
		
		if (fruit == null) {
			fruit = new Jxkh_Fruit();
			fruit.setSubmitName(user.getKuName());
			fruit.setSubmitId(user.getKuLid());
			fruit.setSubmitTime(ConvertUtil.convertDateAndTimeString(new Date()));
		}
		if (isChooseFruit) {
			fruit.setId(null);
		...
		if (firstSignTrue.isChecked()) {
			fruit.setFirstSign(Jxkh_Fruit.YES);
		} else {
			fruit.setFirstSign(Jxkh_Fruit.NO);
		}

		fruit.setTempState("0000000");// 为临时状态赋初值
		fruit.setDep1Reason("");
		fruit.setDep2Reason("");
		fruit.setDep3Reason("");
		fruit.setDep4Reason("");
		...
		Set<Jxkh_FruitFile> oldFile = fruit.getFruitFile();
		if (oldFile != null) {
	...
	public void onClick$chooseProject() throws SuspendNotAllowedException, InterruptedException {
		final ChooserProjectWin win = (ChooserProjectWin) Executions.createComponents("/admin/personal/businessdata/fruit/chooseProject.zul", null, null);
	...
	public void onClick$chooseFruit() throws SuspendNotAllowedException, InterruptedException {
		final ChooseFruitWin win = (ChooseFruitWin) Executions.createComponents("/admin/personal/businessdata/fruit/choosefruit.zul", null, null);
		win.addEventListener(Events.ON_CHANGE, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				fruit = win.getFruit();
				isChooseFruit = true;
