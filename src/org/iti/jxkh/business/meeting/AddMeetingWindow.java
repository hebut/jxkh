...
public class AddMeetingWindow extends Window implements AfterCompose {
	...
	private Toolbarbutton /* chooseMeet, */tempSave, submit, close, submitScore, ups1, ups2, ups3;
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (meeting != null) {
					if (detail == "Detail") {
						...
						if (meeting.getMtState() == null || meeting.getMtState() == 0 || meeting.getMtState() == 3 || meeting.getMtState() == 5 || meeting.getMtState() == 8) {
				...
				if (meeting != null) {
					if (detail == "Detail") {
						...
						if (meeting.getMtState() != null
								&& (meeting.getMtState().equals(JXKH_MEETING.DEPT_PASS) || meeting.getMtState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS) || meeting.getMtState().equals(JXKH_MEETING.BUSINESS_PASS) || meeting.getMtState().equals(
										JXKH_MEETING.SAVE_RECORD))) {
							tempSave.setVisible(false);
							submit.setVisible(false);
						} else {
	...
	public void initWindow() {
		if (meeting == null) {
			...
			// 判断如果是未审核状态则暂存按钮不显示
			if (meeting.getMtState() == null || meeting.getMtState() == JXKH_MEETING.WRITING || meeting.getMtState() == Jxkh_Project.BUSINESS_NOT_PASS || meeting.getMtState() == Jxkh_Project.DEPT_NOT_PASS)
				tempSave.setVisible(true);
			else
				tempSave.setVisible(false);
			if (detail == "Detail") {
				...
				if (meeting.getMtState() == JXKH_MEETING.NOT_AUDIT || meeting.getMtState() == JXKH_MEETING.DEPT_NOT_PASS || meeting.getMtState() == JXKH_MEETING.BUSINESS_NOT_PASS || meeting.getMtState().equals(JXKH_MEETING.WRITING)) {
					submit.setVisible(true);
			...
			// 附件的初始化
			// filesList2 = jxkhMeetingService.findFilesByIdAndType(meeting,
			// "会议照片");
			// Object[] file2 = filesList2.toArray();
			// for (int j = 0; j < file2.length; j++) {
			// JXKH_MeetingFile f = (JXKH_MeetingFile) file2[j];
			...
			// }
			// }
			// uploadbox2.setFileModel(fileModel2);// 此处不能省略，不然编辑时无法删除
			// uploadbox2.initListbox(fileModel2);
			//
			// filesList1 = jxkhMeetingService.findFilesByIdAndType(meeting,
			// "会议通知");
			// Object[] file1 = filesList1.toArray();
			...
			// }
			// }
			// uploadbox1.setFileModel(fileModel1);// 此处不能省略，不然编辑时无法删除
			// uploadbox1.initListbox(fileModel1);
			//
			// filesList = jxkhMeetingService
			// .findFilesByIdAndType(meeting, "会议总结");
			// Object[] file = filesList.toArray();
			// for (int j = 0; j < file.length; j++) {
			// JXKH_MeetingFile f = (JXKH_MeetingFile) file[j];
			// File fileout = new File(Executions.getCurrent().getDesktop()
			// .getWebApp().getRealPath("/upload/")
			// + f.getPath());
			// try {
			...
			// }
			// }
			// uploadbox.setFileModel(fileModel);// 此处不能省略，不然编辑时无法删除
			// uploadbox.initListbox(fileModel);
	...
	public void type() {
		holdCategray.addEventListener(Events.ON_SELECT, new EventListener() {
			public void onEvent(Event event) throws Exception {
				if (((Jxkh_BusinessIndicator) holdCategray.getSelectedItem().getValue()).getKbName() == null 
						|| ((Jxkh_BusinessIndicator) holdCategray.getSelectedItem().getValue()).getKbName().equals(null)
						|| ((Jxkh_BusinessIndicator) holdCategray.getSelectedItem().getValue()).getKbName().equals("--请选择--")) {
						zcRow.setVisible(false);
						zxRow.setVisible(false);
						cxRow.setVisible(false);
	...
	public void onClick$submit() {
		...
		if (i == 0) {
			try {
				Messagebox.show("您不是会议组织人员，不能进行录入！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			return;
		...
		try {
			Messagebox.show("学术会议已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			meetingMemberList = jxkhMeetingService.findMeetingMemberByMeetingId(meeting);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			List<JXKH_MeetingMember> mlist = meeting.getMeetingAuthor();
			List<JXKH_MeetingDept> dlist = meeting.getMtDep();
	...
	public void onClick$tempSave() {
		...
		try {
			Messagebox.show("学术会议保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			meetingMemberList = jxkhMeetingService.findMeetingMemberByMeetingId(meeting);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			List<JXKH_MeetingMember> mlist = meeting.getMeetingAuthor();
