...
public class AddPatentWindow extends Window implements AfterCompose {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (project != null) {
					if (dept1 == "dept" || dept == "dept") {// 部门添加或者部门编辑_
						...
						Jxkh_PatentDept d = jxkhProjectService.findPatentDept(project, user.getDept().getKdNumber());
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (project != null) {
					if (project.getState() != null
							&& (project.getState().equals(JXKH_MEETING.DEPT_PASS) || project.getState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS) || project.getState().equals(JXKH_MEETING.BUSINESS_PASS) || project.getState().equals(JXKH_MEETING.SAVE_RECORD))) {
						tempSave.setVisible(false);
						submit.setVisible(false);
					} else {
						tempSave.setVisible(true);
					...
					if (dept1 == "dept" || dept == "dept") {// 部门添加或者部门编辑_判断当前登录人员是否是信息填写人的部门负责人
						List<WkTUser> wktUser = jxkhMeetingService.findWkTUserByManId(project.getInfoWriter());
						WkTUser u = wktUser.get(0);
	...
	public void initWindow() {
		...
		if (project.getState() == null || project.getState() == JXKH_MEETING.WRITING || project.getState() == Jxkh_Project.BUSINESS_NOT_PASS || project.getState() == Jxkh_Project.DEPT_NOT_PASS)
			...
			tempSave.setVisible(false);
		if (project.getState() == Jxkh_Patent.NOT_AUDIT || project.getState() == Jxkh_Patent.DEPT_NOT_PASS || project.getState() == Jxkh_Patent.BUSINESS_NOT_PASS || project.getState().equals(JXKH_MEETING.WRITING)) {
			submit.setVisible(true);
		...
		// for (File file : fileArray) {
		// oldFiles3.add(file.getName());
		// files3.add(file.getName());
		// }
		// initFileListbox();
		// }
	...
	// public void initFileListbox() {
	// if (files != null) {
	// delet1.setVisible(true);
	// fileListbox1.setModel(new ListModelList(files));
	// }
	// if (files.size() == 0) {
	// delet1.setVisible(false);
	// }
	// if (files2 != null) {
	// delet2.setVisible(true);
	// fileListbox2.setModel(new ListModelList(files2));
	// }
	// if (files2.size() == 0) {
	// delet2.setVisible(false);
	// }
	// if (files3 != null) {
	// delet3.setVisible(true);
	// fileListbox3.setModel(new ListModelList(files3));
	// }
	// if (files3.size() == 0) {
	// delet3.setVisible(false);
	// }
	// }
	...
	public void onClick$tempSave() throws InterruptedException {
		...
		if (dept1 == "dept" || dept == "dept") {// 部门添加或者部门编辑
			...
			if (t == 0) {
				try {
					Messagebox.show("您不是发明人，不能进行录入！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		...
		try {
			...
			Messagebox.show("专利(软件)保存成功，如果您想提交到部门，请点击“提交”按钮！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			projectMemberList = jxkhProjectService.findPatentMember(project);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
	...
	public void onClick$submit() throws InterruptedException {
		...
		if (dept1 != "dept") {
			project.setState(Jxkh_Patent.NOT_AUDIT);
		} else {
			project.setState(Jxkh_Patent.NOT_AUDIT);
		}
		project.setjxYear(jiFenTime.getSelectedItem().getValue() + "");
		...
		try {
			jxkhProjectService.saveOrUpdate(project);
			Messagebox.show("专利(软件)已提交到部门，请耐心等待部门负责人审核！", "提示", Messagebox.OK, Messagebox.INFORMATION);
			projectMemberList = jxkhProjectService.findPatentMember(project);
			scoreTab.setDisabled(false);
			fileTab.setDisabled(false);
			personScore.setModel(new ListModelList(mlist));
