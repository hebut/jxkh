...
public class PatentWindow extends Window implements AfterCompose {
	...
	public void initShow() {
		// List<Jxkh_Patent> patentList = jxkhProjectService.findAllPatent();
		List<Jxkh_Patent> patentList = jxkhProjectService
				.findPatentBymemberId(user.getKuLid());
					...
					public void onEvent(Event evt) throws InterruptedException {
						if (evt.getName().equals("onOK")) {
							...
							while (it.hasNext()) {
								...
								Jxkh_Patent project = (Jxkh_Patent) item
										.getValue();
								...
								if (project.getState() == Jxkh_Patent.DEPT_PASS
										...
										|| project.getState() == Jxkh_Patent.BUSINESS_PASS || project.getState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS)
										|| project.getState() == Jxkh_Patent.SAVE_RECORD) {
									try {
