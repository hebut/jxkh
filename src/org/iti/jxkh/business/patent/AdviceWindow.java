...
public class AdviceWindow extends Window implements AfterCompose {
	...
	private Textbox advice1, advice2, advice3, advice4, advice5, advice6,
			advice7, bAdvice;
	private Jxkh_Patent meeting;
	private JxkhProjectService jxkhProjectService;
	private List<Jxkh_PatentDept> meetDeptList = new ArrayList<Jxkh_PatentDept>();

	public Jxkh_Patent getMeeting() {
		return meeting;
	}

	public void setMeeting(Jxkh_Patent meeting) {
	...
	public void initWindow() {
		// 控制显示多少行，和每行的内容
		meetDeptList = jxkhProjectService.findPatentDep(meeting);
		for (int i = 1; i <= meetDeptList.size(); i++) {
			Jxkh_PatentDept dept = (Jxkh_PatentDept) meetDeptList.get(i - 1);
			switch (i) {
			case 1:
				r1.setVisible(true);
