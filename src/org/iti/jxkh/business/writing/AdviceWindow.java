...
public class AdviceWindow extends Window implements AfterCompose {
	 ...
	 * @SongYu C
	 */
	private static final long serialVersionUID = 5820111961070117626L;
	...
	private Textbox advice1, advice2, advice3, advice4, advice5, advice6,
			advice7, bAdvice;
	private Jxkh_Writing meeting;
	private JxkhProjectService jxkhProjectService;
	private List<Jxkh_WritingDept> meetDeptList = new ArrayList<Jxkh_WritingDept>();

	public Jxkh_Writing getMeeting() {
		return meeting;
	}

	public void setMeeting(Jxkh_Writing meeting) {
	...
	public void initWindow() {
		// 控制显示多少行，和每行的内容
		meetDeptList = jxkhProjectService.findWritingDep(meeting);
		for (int i = 1; i <= meetDeptList.size(); i++) {
			Jxkh_WritingDept dept = (Jxkh_WritingDept) meetDeptList.get(i - 1);
			switch (i) {
			case 1:
				r1.setVisible(true);
