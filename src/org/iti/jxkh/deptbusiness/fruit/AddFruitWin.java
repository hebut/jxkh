...
public class AddFruitWin extends Window implements AfterCompose {
	...
	public void afterCompose() {
		...
		scoreTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (fruit != null) {
					...
					List<WkTUser> wktUser = jxkhMeetingService.findWkTUserByManId(fruit.getSubmitId());
					WkTUser u = wktUser.get(0);
					Jxkh_FruitDept d = jxkhFruitService.findFruitDept(fruit, user.getDept().getKdNumber());
					if (user.getDept().getKdNumber().equals(u.getDept().getKdNumber()) || d.getRank() == 1) {
						if (fruit.getState() == null || fruit.getState() == 0 || fruit.getState() == 3 || fruit.getState() == 5) {
		...
		baseTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (fruit != null) {
					if (fruit.getState() == null || fruit.getState() == 0 || fruit.getState().shortValue() == JXKH_MEETING.WRITING || fruit.getState() == 3 || fruit.getState() == 5) {
						save.setVisible(true);
						tempSave.setVisible(true);
					} else {
						save.setVisible(false);
	 ...
	 * "/admin/personal/deptbusinessdata/fruit/chooseProject.zul", null, null);
	...
	public class personScoreRenderer implements ListitemRenderer {
		...
		public void render(Listitem item, Object data) throws Exception {
			...
			final List<Jxkh_FruitDept> temp = jxkhFruitService.findFruitDepByFruitId(fruit); // 弹出指定部门页面
	...
	public void onClick$chooseFruit() throws SuspendNotAllowedException, InterruptedException {
		final ChooseFruitWin win = (ChooseFruitWin) Executions.createComponents("/admin/personal/deptbusinessdata/fruit/choosefruit.zul", null, null);
		win.addEventListener(Events.ON_CHANGE, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				fruit = win.getFruit();
				isChooseFruit = true;
