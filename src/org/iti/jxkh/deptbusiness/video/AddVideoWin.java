...
public class AddVideoWin extends Window implements AfterCompose {
	...
	public void afterCompose() {
		...
		scoreTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (video != null) {
					...
					List<WkTUser> wktUser = jxkhMeetingService.findWkTUserByManId(video.getSubmitId());
					WkTUser u = wktUser.get(0);
					Jxkh_VideoDept d = jxkhVideoService.findVideoDept(video, user.getDept().getKdNumber());
					if (user.getDept().getKdNumber().equals(u.getDept().getKdNumber()) || d.getRank() == 1) {
						if (video.getState() == null || video.getState() == 0 || video.getState() == 3 || video.getState() == 5) {
		...
		baseTab.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (video != null) {
					if (video.getState() == null || video.getState() == 0 || video.getState().shortValue() == JXKH_MEETING.WRITING || video.getState() == 3 || video.getState() == 5) {
						save.setVisible(true);
						tempSave.setVisible(true);
					} else {
						save.setVisible(false);
	...
	public void onClick$save() {
		try {
			...
			video.setState(Jxkh_Report.NOT_AUDIT);
			jxkhVideoService.saveOrUpdate(video);
			Messagebox.show("影视保存成功！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		} catch (Exception e) {
			e.printStackTrace();
