...
public class AdviceWindow extends Window implements AfterCompose {
	 ...
	 * @author CXX
	 */
	private static final long serialVersionUID = -71186220204868597L;
	...
	public void initWindow() {
		...
		// 控制显示多少行，和每行的内容
		meetDeptList = jxkhHylwService.findMeetingDepByMeetingId(meeting);
		for (int i = 1; i <= meetDeptList.size(); i++) {
			JXKH_HYLWDept dept = (JXKH_HYLWDept) meetDeptList.get(i - 1);
			switch (i) {
			case 1:
				r1.setVisible(true);
		...
		if (meeting.getLwState() == 1 || meeting.getLwState() == 4
				|| meeting.getLwState() == 6 || meeting.getLwState() == JXKH_MEETING.BUSINESS_TEMP_PASS) {
