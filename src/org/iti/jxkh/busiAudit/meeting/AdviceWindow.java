...
public class AdviceWindow extends Window implements AfterCompose {
	...
	public void initWindow() {
		if (meeting.getMtState() == 1 || meeting.getMtState() == 4
				|| meeting.getMtState() == 5 || meeting.getMtState() == 7) {
			pass.setDisabled(false);
			back.setDisabled(false);
			temp.setDisabled(false);
			bAdvice.setDisabled(false);
		}
		if (meeting.getMtState() == 4|| meeting.getMtState() == 6) {
			pass.setSelected(true);
		}else if (meeting.getMtState() == 5) {
			back.setSelected(true);
		}else if(meeting.getMtState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS))
	...
	public void onClick$save() {
		...
		if (pass.isChecked()) {			
			meeting.setMtState(JXKH_MEETING.BUSINESS_PASS);
		} else if (temp.isChecked()) {
			meeting.setMtState(JXKH_MEETING.BUSINESS_TEMP_PASS);
		}else {
			meeting.setMtState(JXKH_MEETING.BUSINESS_NOT_PASS);
		}
