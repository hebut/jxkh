...
public class MeetingDetailWindow extends Window implements AfterCompose {
	...
	private Row coUnitRow, zcRow, zxRow, cxRow, cordId;
	private Textbox zdep1, zdep2, cdep1, cdep2, xdep1, xdep2;
	private Toolbarbutton /* chooseMeet, */close, submit;
	...
	public void afterCompose() {
		...
		// Events.postEvent(Events.ON_SELECT, holdCategray ,null);
		type();
	...
	public void initWindow() {
		if (meeting == null) {
			...
			// 归档后显示档案号
			if (meeting.getMtState() == null || meeting.getMtState() == 0 || meeting.getMtState() == 1 || meeting.getMtState() == 2 || meeting.getMtState() == 3 || meeting.getMtState() == 4 || meeting.getMtState() == 5) {
				record.setVisible(false);
				cordId.setVisible(false);
			} else if (meeting.getMtState() == 6) {
	...
	public void onClick$submit() {
		...
		try {
			...
			meetingMemberList = meeting.getMeetingAuthor();
			meetDeptList = meeting.getMtDep();
			personScore.setModel(new ListModelList(meetingMemberList));
			departmentScore.setModel(new ListModelList(meetDeptList));
	...
	public void setEntity() {
		...
		meeting.setRecordCode(record.getValue());// 修改档案号
		...
		Set<JXKH_MeetingFile> oldFile = new HashSet<JXKH_MeetingFile>();
