...
public class AdviceWin extends Window implements AfterCompose {
	 ...
	 * @author ZhangYuguang
	 */
	private static final long serialVersionUID = 3346817778431005457L;
	...
	public void initWindow() {
		...
		}else if(meeting.getState().equals(JXKH_MEETING.BUSINESS_TEMP_PASS))
			temp.setSelected(true);
		// 控制显示多少行，和每行的内容
		meetDeptList = jxkhVideoService.findVideoDeprByVideoId(meeting);
		for (int i = 1; i <= meetDeptList.size(); i++) {
			Jxkh_VideoDept dept = (Jxkh_VideoDept) meetDeptList.get(i - 1);
			switch (i) {
			case 1:
				r1.setVisible(true);
	...
	public void onClick$save() {
		...
		try {
			jxkhVideoService.saveOrUpdate(meeting);
