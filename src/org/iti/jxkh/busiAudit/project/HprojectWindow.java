...
public class HprojectWindow extends Window implements AfterCompose {
	...
	private Listbox hxListbox, auditState2, rank2, dept2;
	WkTUser user;
	Boolean cx2;
	...
	public void initShow() {
		...
		String[] a = { "", "待审核", "部门审核中", "部门通过", "部门不通过", "业务办暂缓通过", "业务办通过", "业务办不通过", "归档" };
		...
		public void render(Listitem item, Object data) throws Exception {
			...
			c2.addEventListener(Events.ON_CLICK, new EventListener() {
				public void onEvent(Event arg0) throws Exception {
					...
					AddHPWindow w = (AddHPWindow) Executions.createComponents("/admin/jxkh/busiAudit/project/addHP.zul", null, null);
					try {
						w.setTitle("编辑横向项目信息");
			...
			download.addEventListener(Events.ON_CLICK, new EventListener() {
				public void onEvent(Event arg0) throws Exception {
					DownloadWindow win = (DownloadWindow) Executions.createComponents("/admin/jxkh/busiAudit/award/download.zul", null, null);
			...
			Image addrecode = new Image("/css/default/images/button/actEdit.gif");
			addrecode.addEventListener(Events.ON_CLICK, new EventListener() {
				public void onEvent(Event arg0) throws Exception {
					if (project.getState() == 0 
							...
							|| project.getState() == 7) {
						try {
							Messagebox.show("业务办还未审核通过，不能进行归档！", "提示", Messagebox.OK, Messagebox.INFORMATION);
						} catch (InterruptedException e) {
							e.printStackTrace();
						}
						return;
					}
					RecordCodeWin win = (RecordCodeWin) Executions.createComponents("/admin/jxkh/busiAudit/project/recordcode.zul", null, null);
			...
			c8.addEventListener(Events.ON_CLICK, new EventListener() {
				public void onEvent(Event event) throws Exception {
					AdviceWin w = (AdviceWin) Executions.createComponents("/admin/jxkh/busiAudit/project/advice.zul", null, null);
					try {
	...
	public void onClick$passAll2() {
		...
		BatchAuditWin win = (BatchAuditWin) Executions.createComponents("/admin/jxkh/busiAudit/project/batchAudit.zul", null, null);
		try {
	...
	public void onClick$query2() {
		...
		List<Jxkh_Project> projectList = jxkhProjectService.findHPByCondition(nameSearch, auditStateSearch, indicatorId, depName);
