 ...
 * 
 * @author DaLei
 * @version $Id: OrganizeLeftTreeComposer.java,v 1.1 2011/08/31 07:03:00 ljb Exp $
 */
public class OrganizeLeftTreeComposer extends BaseLeftTreeComposer {
	Tree organizeTree;
	WkTUser user;
	// 当前三级标题树的父标题编号
	Long tid;
		...
		organizeTree.addEventListener(Events.ON_SELECT, new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				...
				if (item.getValue() instanceof WkTDept) {
					...
					Component c = (Component) creatTab("organize", "管理组织单位", "/admin/human/organize/manager.zul", organizePanel);
					if (c instanceof OrganizeManagerWindow) {
						OrganizeManagerWindow baseWindow = (OrganizeManagerWindow) c;
