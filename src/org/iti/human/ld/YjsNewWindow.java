 ...
 * @date 2010-3-18
 */
public class YjsNewWindow extends Window implements AfterCompose {
	...
	public void onClick$submit() throws InterruptedException {
		...
		List slist = userService.getUsersByLogin(sid);
		for (int i = 0; i < slist.size(); i++) {
			WkTUser yjs = (WkTUser) slist.get(i);
			if (yjs.getDept().getKdSchid().longValue() == getDept().getKdSchid().longValue()) {
				throw new WrongValueException(stid, "学号已经存在，与" + yjs.getKuName() + "相同");
			}
		}
		userService.save(editUser);
		System.out.println(dept.getKdId());
		Yjs stu = new Yjs();
		stu.setYjsId(stid.getValue());
		stu.setKuId(editUser.getKuId());
		stu.setClId(stClassList.getSelectClass().getClId());
		stu.setYjsGrade(stClassList.getSelectClass().getClGrade());
		Short stNormal = 0;
		...
		stu.setYjsNormal(stNormal);
		stu.setYjsBynf(getBynf());
		yjsService.save(stu);
