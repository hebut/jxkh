...
public class ArticalListWindow extends Window implements AfterCompose {

	private static final long serialVersionUID = 3255000106611398024L;
	
	ArticalListWindow articallistWindow;
	WkTUser user;
	private GhXm xm;
	...
	public void initWindow()
	{
		if(member.getKuId().equals(user.getKuId()))
			add.setVisible(true);
		List<Long> countList = archiveService.getCountByKyIdAndKuId(xm.getKyId(),member.getKuId(),GhXm.KYXM);
		int articalNum = 0;
		...
		List<GH_ARCHIVE> archiveList = archiveService.findByKyIdAndUserIdAndPage(xm.getKyId(), member.getKuId(),GhXm.KYXM, 0, arPaging.getPageSize());
	...
	{		
		NewArticalWindow w=(NewArticalWindow)
			Executions.createComponents("/admin/personal/data/teacherinfo/kyqk/member/newartical.zul",null,null);
		w.setXm(xm);
		w.setArticallistWindow(articallistWindow);
		w.initWindow();
		try {
			w.doModal();
	...
	public class EditEventListener implements EventListener 
	{
		public void onEvent(Event event) throws Exception 
		{
			Listitem item = (Listitem)event.getTarget().getParent().getParent().getParent();
			GH_ARCHIVE archive = (GH_ARCHIVE)item.getValue();
			ArticalEditWindow w=(ArticalEditWindow)
				Executions.createComponents("/admin/personal/data/teacherinfo/kyqk/member/articalEdit.zul",null,null);
			w.setXm(xm);
	...
	public class PagingListener implements EventListener {
		...
		public void onEvent(Event event) throws Exception {
			List<GH_ARCHIVE> archiveList = archiveService.findByKyIdAndUserIdAndPage(xm.getKyId(),member.getKuId(),GhXm.KYXM,arPaging.getActivePage(),arPaging.getPageSize());
			ListModel articalListModel = new ListModelList(archiveList);
			articalListbox.setModel(articalListModel);
		}
