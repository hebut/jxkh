...
//	private static final long serialVersionUID = 1L;
	...
	StageEditWindow w=(StageEditWindow)Executions.createComponents("/admin/personal/data/teacherinfo/kyqk/member/stageedit.zul", null, null);
	...
	public void initShow()
	{
		...
		List list1 = xmService.findCountByKuidAndType(user.getKuId(), GhJsxm.KYXM);
		List list2= xmService.findByKuidAndType(user.getKuId(), GhJsxm.KYXM,xmlistPaging.getActivePage(), xmlistPaging.getPageSize());
		if(list1.size()!=0)
		{
		int size=((Long) list1.get(0)).intValue();
		xmlistPaging.setTotalSize(size);	
		}
		project.setModel(new ListModelList(list2));
		xmlistPaging.addEventListener("onPaging", new EventListener() {
			public void onEvent(Event arg0) throws Exception {
				List xmlist= xmService.findByKuidAndType(user.getKuId(), GhJsxm.KYXM,xmlistPaging.getActivePage(), xmlistPaging.getPageSize());
				project.setModel(new ListModelList(xmlist));				
			}
		});
		project.setItemRenderer(new ListitemRenderer() {
