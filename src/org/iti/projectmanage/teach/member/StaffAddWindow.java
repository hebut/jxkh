...
public class StaffAddWindow extends Window implements AfterCompose{
	...
	Listitem cout;
	Listbox range,jxlistbox1,jxlistbox2;
	DeptListbox dept;
  ...
  public void initWindow(GhXm xm)
  {
   ...
   public void onClick$outadd() throws SuspendNotAllowedException, InterruptedException
   {
	   ...
	   Executions.createComponents("/admin/personal/data/teacherinfo/jxqk/member/addoutmember.zul", null, null);
	   w.initWindow(xm);
  ...
  public void initOutMember()
  {
	  ...
	  if(outmlist.size()!=0)
	  {
		  jxlistbox1.setModel(new ListModelList(outmlist));  
		  jxlistbox1.setItemRenderer(new ListitemRenderer() {
  ...
  public void initMember()
  {
	  ...
	  if(member.size()!=0)
	  {
	  jxlistbox2.setModel(new ListModelList(member));
	  
		  jxlistbox2.setItemRenderer(new ListitemRenderer() {
				public void render(Listitem arg0, Object arg1) throws Exception {
  ...
  { 
	  WkTDept deptment = (WkTDept) dept.getSelectedItem().getValue();
	   List<WkTUser> userlist =userService.findUserForGroupByKdIdAndName(deptment.getKdId(), name.getValue(), teacher.isChecked(), student.isChecked(), master.isChecked());	
		jxlistbox1.setModel(new ListModelList(userlist));
		jxlistbox1.setItemRenderer(new ListitemRenderer() {
			public void render(Listitem arg0, Object arg1) throws Exception {
  ...
  public void onClick$add()
  {
		...
		while (jxlistbox1.getSelectedItem() != null) {
			Listitem item = jxlistbox1.getSelectedItem();
			...
			else
			{
			  ...
			  List mem=projectmemberService.findByKyXmId(xm.getKyId());
				...
				if (jxlistbox1.getItemCount() != 0)
					jxlistbox2.insertBefore(item, jxlistbox1.getItemAtIndex(0));
				else
					item.setParent(jxlistbox2);
...
//删除成员
  public void onClick$delete() throws InterruptedException
  {
		...
		while (jxlistbox2.getSelectedItem() != null) {
			Listitem item = jxlistbox2.getSelectedItem();
			if(item.getValue() instanceof WkTUser)
			{
			WkTUser user = (WkTUser) item.getValue();
					...
					{  
					 List memberlist = projectmemberService.findByKyIdAndKuId(xm.getKyId(), user.getKuId());
						if (memberlist.size() != 0) {
							...
							if (jxlistbox2.getItemCount() != 0)
								jxlistbox1.insertBefore(item, jxlistbox2.getItemAtIndex(0));
							else
								item.setParent(jxlistbox1);
							item.setSelected(false);
						}
					}
				 else return;
