...
import com.uniwin.framework.security.SecurityUtil;
...
public class Executions {
	...
	/**
	 * Returns the evaluator of the current execution. It is usually used to parse the expression into {@link org.zkoss.xel.Expression} or used with {@link org.zkoss.zk.xel.ExValue}. for performance improvement.
	 * 
	 * @param page
	 *            the page that this evaluator is associated. If null, the current page and then the first page is assumed.
	 * @param expfcls
	 *            the implementation of {@link org.zkoss.xel.ExpressionFactory}, or null to use the default ({@link org.zkoss.zk.ui.util.Configuration#getExpressionFactoryClass}.
	 * @since 3.0.0
	 */
	...
	/**
	 * Evluates the specified expression by use of the current context ({@link #getCurrent}).
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * For better performance, you can use the instance returned by {@link #getEvaluator} to parse and cached the parsed expression. {@link org.zkoss.zk.xel.ExValue} is a utility class to simply the task.
	 * 
	 * @param comp
	 *            as the self variable (ignored if null)
	 */
	...
	/**
	 * Evluates the specified expression with the resolver of the current execution ({@link #getCurrent}).
	 * 
	 * <p>
	 ...
	 * 
	 * @param page
	 *            used as the self variable and to retrieve the function mapper if funmap is not defined. Ignored if null.
	 */
	 ...
	 * 
	 * <p>
	 * It resolves "*" contained in URI, if any, to the proper Locale, and the browser code. Refer to {@link org.zkoss.web.servlet.Servlets#locate(javax.servlet.ServletContext, ServletRequest, String, Locator)} for details.
	 * 
	...
	/**
	 * Encodes the specified URL into an instance of {@link URL}. It is similar to {@link #encodeURL}, except it returns an instance of {@link URL}.
	 * 
	 * @exception NullPointerException
	 *                if the current execution is not available.
	 * @exception MalformedURLException
	 *                if failed to convert it to a legal {@link URL}
	 * @since 3.5.0
	 */
	public static final URL encodeToURL(String uri) throws MalformedURLException {
		...
		if (uri.indexOf("://") < 0) {
			final StringBuffer sb = new StringBuffer(256).append(exec.getScheme()).append("://").append(exec.getServerName());
	...
	/**
	 * Creates components from a page file specified by an URI. Shortcut to {@link Execution#createComponents(String, Component, Map)}.
	 * 
	 * @param parent
	 *            the parent component, or null if you want it to be a root component. If parent is null, the page is assumed to be the current page, which is determined by the execution context. In other words, the new component will be the root component of the current page if parent is null.
	 * @param arg
	 *            a map of parameters that is accessible by the arg variable in EL, or by {@link Execution#getArg}. Ignored if null.
	...
	public static final Component createComponents(String uri, Component parent, Map arg) {
		if (SecurityUtil.checkPermission(uri, (HttpSession) Sessions.getCurrent().getNativeSession())) {
			return getCurrent().createComponents(uri, parent, arg);
		} else {
			return getCurrent().createComponents("/error.zul", parent, arg);
		}
	}

	/**
	 * Creates components based on the specified page definition. Shortcut to {@link Execution#createComponents(PageDefinition, Component, Map)}.
	 * 
	 * @param pagedef
	 *            the page definition to use. It cannot be null.
	 * @param parent
	 *            the parent component, or null if you want it to be a root component. If parent is null, the page is assumed to be the current page, which is determined by the execution context. In other words, the new component will be the root component of the current page if parent is null.
	 * @param arg
	 *            a map of parameters that is accessible by the arg variable in EL, or by {@link Execution#getArg}. Ignored if null.
	...
	/**
	 * Creates components from the raw content specified by a string. Shortcut to {@link Execution#createComponentsDirectly(String, String, Component, Map)}.
	 * 
	 * @param content
	 *            the raw content of the page. It must be a XML and compliant to the page format (such as ZUL).
	 * @param extension
	 *            the default extension if the content doesn't specify an language. In other words, if the content doesn't specify an language, {@link LanguageDefinition#getByExtension} is called. If extension is null and the content doesn't specify a language, the language called "xul/html" is assumed.
	 * @param parent
	 *            the parent component, or null if you want it to be a root component. If parent is null, the page is assumed to be the current page, which is determined by the execution context. In other words, the new component will be the root component of the current page if parent is null.
	 * @param arg
	 *            a map of parameters that is accessible by the arg variable in EL, or by {@link Execution#getArg}. Ignored if null.
	...
	/**
	 * Creates components from the raw content specified by a DOM tree. Shortcut to {@link Execution#createComponentsDirectly(Document, String, Component, Map)}.
	 * 
	 * @param content
	 *            the raw content in DOM.
	 * @param extension
	 *            the default extension if the content doesn't specify an language. In other words, if the content doesn't specify an language, {@link LanguageDefinition#getByExtension} is called. If extension is null and the content doesn't specify a language, the language called "xul/html" is assumed.
	 * @param parent
	 *            the parent component, or null if you want it to be a root component. If parent is null, the page is assumed to be the current page, which is determined by the execution context. In other words, the new component will be the root component of the current page if parent is null.
	...
	/**
	 * Creates components from the raw content read from the specified reader. Shortcut to {@link Execution#createComponentsDirectly(Reader, String, Component, Map)}.
	 * 
	 * <p>
	 * The raw content is loader and parsed to a page defintion by use of {@link Execution#getPageDefinitionDirectly(Reader, String)}, and then invokes {@link #createComponents(PageDefinition,Component,Map)} to create components.
	 * 
	 * @param reader
	 ...
	 * 
	 * <p>
	 * Unlike {@link #createComponents(PageDefinition,Component,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param pagedef
	 *            the page definition to use. It cannot be null.
	 * @param arg
	 *            a map of parameters that is accessible by the arg variable in EL, or by {@link Execution#getArg}. Ignored if null.
	 ...
	 * 
	 * <p>
	 * It loads the page definition from the specified URI (by use {@link #getPageDefinition} ), and then invokes {@link #createComponents(WebApp,PageDefinition,Map)} to create components.
	 * 
	 * <p>
	 * Unlike {@link #createComponents(String,Component,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param arg
	 ...
	 * 
	 * <p>
	 * The raw content is parsed to a page defintion by use of {@link #getPageDefinitionDirectly(WebApp,String,String)}, and then invokes {@link #createComponents(WebApp,PageDefinition,Map)} to create components.
	 * 
	 * <p>
	 * Unlike {@link #createComponentsDirectly(String,String,Component,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param content
	 *            the raw content of the page. It must be in ZUML.
	 * @param extension
	 *            the default extension if the content doesn't specify an language. In other words, if the content doesn't specify an language, {@link LanguageDefinition#getByExtension} is called. If extension is null and the content doesn't specify a language, the language called "xul/html" is assumed.
	 * @param arg
	 *            a map of parameters that is accessible by the arg variable in EL, or by {@link Execution#getArg}. Ignored if null.
	 ...
	 * 
	 * <p>
	 * The raw content is parsed to a page defintion by use of {@link #getPageDefinitionDirectly(WebApp,Document, String)}, and then invokes {@link #createComponents(WebApp,PageDefinition,Map)} to create components.
	 * 
	 * <p>
	 * Unlike {@link #createComponentsDirectly(Document,String,Component,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param content
	 ...
	 * 
	 * <p>
	 * The raw content is loaded and parsed to a page defintion by use of {@link #getPageDefinitionDirectly(WebApp,Reader,String)}, and then invokes {@link #createComponents(WebApp,PageDefinition,Map)} to create components.
	 * 
	 * <p>
	 * Unlike {@link #createComponentsDirectly(Reader,String,Component,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param reader
	 ...
	 * 
	 * <p>
	 * Like {@link #createComponents(WebApp,PageDefinition,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param uri
	 *            the URI of the page file.
	 * 
	 ...
	 * 
	 * <p>
	 * Like {@link #createComponentsDirectly(WebApp,Document,String,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param content
	 *            the raw content of the page in DOM.
	 * @param extension
	 *            the default extension if the content doesn't specify an language. In other words, if the content doesn't specify an language, {@link LanguageDefinition#getByExtension} is called. If extension is null and the content doesn't specify a language, the language called "xul/html" is assumed.
	 * @see #getPageDefinitionDirectly(WebApp, String, String)
	 * @see #getPageDefinitionDirectly(WebApp, Reader, String)
	 ...
	 * 
	 * <p>
	 * Like {@link #createComponentsDirectly(WebApp,Reader,String,Map)}, this method can be inovked without the current execution, such as a working thread. In this case, the wapp argument must be specified.
	 * 
	 * @param wapp
	 *            the Web application. It is optional and used only if no current execution (e.g., in a working thread).
	 * @param reader
	 *            used to input the raw content of the page. It must be in ZUML.
	 * @param extension
	 *            the default extension if the content doesn't specify an language. In other words, if the content doesn't specify an language, {@link LanguageDefinition#getByExtension} is called. If extension is null and the content doesn't specify a language, the language called "xul/html" is assumed.
	 * @see #getPageDefinitionDirectly(WebApp, String, String)
	 * @see #getPageDefinitionDirectly(WebApp, Document, String)
	...
	/**
	 * Sends a temporary redirect response to the client using the specified redirect location URL by use of the current execution, {@link #getCurrent}.
	 * 
	 * <p>
	...
	// -- wait/notify --//
	/**
	 * Suspends the current processing of an event and wait until the other thread invokes {@link #notify(Object)}, {@link #notifyAll(Object)}, {@link #notify(Desktop, Object)} or {@link #notifyAll(Desktop, Object)} for the specified object.
	 * 
	 * <p>
	 ...
	 * 
	 * @param mutex
	 *            any non-null object to identify what to notify. It must be same object passed to {@link #notify(Desktop, Object)}. If there is racing issue, you have to enclose it with <code>synchronized</code> (though it is optional).
	 * @exception UiException
	 *                if it is called not during event processing.
	 * @exception SuspendNotAllowedException
	 *                if there are too many suspended exceptions. Deployers can control the maximal allowed number of suspended exceptions by specifying <code>max-suspended-thread</code> in <code>zk.xml</code>, or invoking {@link org.zkoss.zk.ui.util.Configuration#setMaxSuspendedThreads}.
	 */
	 ...
	 * 
	 * <p>
	 * Use {@link #notify(Desktop, Object)} if you want to notify in other thread, such as a working thread.
	 * 
	 * @param mutex
	 *            any non-null object to identify what to notify. It must be same object passed to {@link #wait}. If there is racing issue, you have to enclose it with <code>synchronized</code> (though it is optional).
	 ...
	 * 
	 * <p>
	 * Use {@link #notifyAll(Desktop, Object)} if you want to notify in other thread, such as a working thread.
	 * 
	 * @param mutex
	 *            any non-null object to identify what to notify. It must be same object passed to {@link #wait}. If there is racing issue, you have to enclose it with <code>synchronized</code> (though it is optional).
	 * @see #notify(Desktop, Object)
	 ...
	 * 
	 * @param desktop
	 *            the desktop which the suspended thread is processing. It must be the same desktop of the suspended thread.
	 * @param mutex
	 *            any non-null object to identify what to notify. It must be same object passed to {@link #wait}. If there is racing issue, you have to enclose it with <code>synchronized</code> (though it is optional).
	 * @see #notify(Object)
	...
	/**
	 * Activates a server-push thread. It causes the current thread to wait until the desktop is available to access, the desktop no longer exists, or some other thread interrupts this thread.
	 * 
	 * <p>
	 ...
	 * 
	 * <pre>
	 * &lt;code&gt;class MyWorkingThread extends Thread {
	 *  public void run() {
	 ...
	 * @exception DesktopUnavailableException
	...
	/**
	 * Activates a server-push thread with, or until a certain amount of real time has elapsed. It causes the current thread to wait until the desktop is available to access, the desktop no longer exists, some other thread interrupts this thread, or a certain amount of real time has elapsed.
	 * 
	 * @param timeout
	 *            the maximum time to wait in milliseconds. Ingored (i.e., never timeout) if non-positive.
	 * @return whether it is activated or it is timeout. The only reason it returns false is timeout.
	 * @exception InterruptedException
	 *                if it is interrupted by other thread
	 * @exception DesktopUnavailableException
	 *                if the desktop is removed (when activating).
	 * @exception IllegalStateException
	 *                if the server push is not enabled for this desktop yet ({@link Desktop#enableServerPush}).
	 * @since 3.0.0
