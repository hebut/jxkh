...
public class UserLogin3Window extends Div implements AfterCompose {
	...
	Captcha cpa;
	// 登陆及重置按钮
	Button loginButton, resetButton, reRegistButton;
	// 用户数据访问接口
	...
	public void afterCompose() {
		...
		// 设置验证码显示
		cpa.addFont(new Font("TimesNewRoman", Font.BOLD, 18));
		cpa.setWidth("100px");
		cpa.setHeight("20px");
		// cpa.randomValue();
	...
	public void loginSucess(WkTUser user) {
		...
		String filePath = ConvertUtil.getPath("/personalHomePage") + user.getKuLid().trim();
		File file = new File(filePath);
		if (file.isDirectory()) {
			String tempContent = FileUtil.readUTF8(filePath + "\\" + "index.zul");
			String savePath = ConvertUtil.getPath("/personalHomePage");
